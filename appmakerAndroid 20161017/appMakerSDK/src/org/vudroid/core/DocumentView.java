...
public class DocumentView extends View implements ZoomListener
{
    ...
    public DocumentView(Context context, DecodingProgressModel progressModel,
    ...
    protected void initMultiTouchZoomIfAvailable(ZoomModel zoomModel)
    {
        try
        {
            multiTouchZoom = (MultiTouchZoom)Class.forName("org.vudroid.core.multitouch.MultiTouchZoomImpl")
                                                  .getConstructor(ZoomModel.class)
                                                  .newInstance(zoomModel);
        } catch (Exception e)
        {
    ...
    private void goToPageImpl(final int toPage)
    {
        ...
        final Page page = pages.get(toPage);
    ...
    public boolean onTouchEvent(MotionEvent ev)
    {
        ...
        case MotionEvent.ACTION_UP:
            ...
            scroller.fling(getScrollX(), getScrollY(), (int)-velocityTracker.getXVelocity(),
                           (int)-velocityTracker.getYVelocity(), getLeftLimit(), getRightLimit(),
    ...
    private void lineByLineMoveTo(int direction)
    {
        if (direction == 1 ? getScrollX() == getRightLimit() : getScrollX() == getLeftLimit())
        {
            scroller.startScroll(getScrollX(),
                                 getScrollY(),
                                 direction * (getLeftLimit() - getRightLimit()),
    ...
    private int getTopLimit()
    {
        ...
        int topDelt = (int)((getHeight() - pages.get(pageIndex).bounds.height()) / 2);
        if(topDelt > 0) top += topDelt;
        return top;
    }
    ...
    private int getBottomLimit()
    {
        ...
        int bottom = (int)pages.get(getCurrentPage()).getBottom() - getHeight();
        int bottomDelt = (int)((getHeight() - pages.get(pageIndex).bounds.height()) / 2);
        if(bottomDelt > 0) bottom += bottomDelt;
        return bottom;
    }
    ...
    RectF getViewRect()
    {
        if (viewRect == null)
        {
            viewRect = new RectF(getScrollX(), getScrollY(), getScrollX() + getWidth(),
    ...
    protected void savePage(Page page)
    {
        ...
        if (cacheDir == null || bitmap == null)
            return;
        File file;
        /*boolean isPortraitBitmap;
        if(this.isPortrait)
        {
            if(bitmap.getWidth() == getWidth())
                isPortraitBitmap = true;
            else
                isPortraitBitmap = false;
        }
        else
        {
            if(bitmap.getWidth() == getWidth())
