...
public class AdapterViewPageVer extends AdapterView<Adapter> implements
	...
	public AdapterViewPageVer(Context context, AttributeSet attrs) {
		super(context, attrs);
		mGestureDetector = new GestureDetector(this);
		mScaleGestureDetector = new ScaleGestureDetector(context, this);
		mScroller = new Scroller(context);
	...
	public AdapterViewPageVer(Context context, AttributeSet attrs, int defStyle) {
	...
	protected void onLayout(boolean changed, int left, int top, int right,
		...
		if (!mResetLayout) {
			...
			if (cv != null) {
				...
				if (cv.getTop() + cv.getMeasuredHeight() + cvOffset.y + GAP / 2
						+ mYScroll < getHeight() / 2
				...
				if (cv.getTop() - cvOffset.y - GAP / 2 + mYScroll >= getHeight() / 2
						&& mCurrent > 0) {
					postUnsettle(cv);
					// post to invoke test for end of animation
					// where we must set hq area for the new current view
		...
		if (mCurrent > 0) {
			...
			lv.layout((cvLeft + cvRight - lv.getMeasuredWidth()) / 2, cvTop
					- gap - lv.getMeasuredHeight(),
		...
		if (mCurrent + 1 < mAdapter.getCount()) {
			...
			rv.layout((cvLeft + cvRight - rv.getMeasuredWidth()) / 2,
					(cvBottom + gap),
					(cvLeft + cvRight + rv.getMeasuredWidth()) / 2, (cvBottom
							+ rv.getMeasuredHeight() + gap));
		}
