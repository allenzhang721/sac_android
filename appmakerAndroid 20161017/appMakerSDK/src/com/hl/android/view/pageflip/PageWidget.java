...
/*	private void calcPath0(){
	...
	private void drawCurrentPageArea(Canvas canvas, Bitmap bitmap, Path path) {
		...
		canvas.drawBitmap(bitmap, null, new RectF(0,0,BookSetting.SCREEN_WIDTH,BookSetting.SCREEN_HEIGHT), null);
		canvas.restore();
	}
	...
	protected void onDraw(Canvas canvas) {
		try {
			if (this.isRight == true) {
				...
				if(_preload){
					mNextPageBitmap = BookController.getInstance().getSnapshot(BookController.getInstance().getViewPage());
				}
				...
				if(mNextPageBitmap != null){
					...
					//绘制下一页的卷曲阴影
					drawNextPageAreaAndShadow0(canvas, mNextPageBitmap);
				}
				...
				//绘制下一页的卷曲阴影
				drawNextPageAreaAndShadow0(canvas, mCurPageBitmap);
				//drawCurrentPageShadow0(canvas);
			}
		} catch (Exception e) {
			e.printStackTrace();
...
//	}
	...
	private void startAnimation(int delayMillis) {
		...
		if (this.isRight == true) {
			...
			mCornerX = BookSetting.SCREEN_WIDTH;
			mCornerY = BookSetting.SCREEN_HEIGHT;
			
			mScroller.startScroll(-BookSetting.SCREEN_WIDTH + 10, BookSetting.SCREEN_HEIGHT - 1, BookSetting.SCREEN_WIDTH * 2, 0, delayMillis);
		}
	...
	public void play(int pageIndex,int newPageIndex,ActionOnEnd action) {
		...
		mWidth = BookSetting.SCREEN_WIDTH;
		mHeight = BookSetting.SCREEN_HEIGHT;
		...
		if (isRight) {//currentindex == -1
			...
			this.calcCornerXY(BookSetting.SCREEN_WIDTH,
					BookSetting.SCREEN_HEIGHT);

			this.doTouchEvent(MotionEvent.obtain(100, 100,
					MotionEvent.ACTION_DOWN, BookSetting.SCREEN_WIDTH,
					BookSetting.SCREEN_HEIGHT, 0));
			this.doTouchEvent(MotionEvent.obtain(100, 100,
					MotionEvent.ACTION_MOVE, BookSetting.SCREEN_WIDTH - 30,
					BookSetting.SCREEN_HEIGHT - 10, 0));
			this.doTouchEvent(MotionEvent.obtain(100, 100,
					MotionEvent.ACTION_MOVE, BookSetting.SCREEN_WIDTH - 50,
					BookSetting.SCREEN_HEIGHT - 20, 0));
			this.doTouchEvent(MotionEvent.obtain(100, 100,
					MotionEvent.ACTION_UP, BookSetting.SCREEN_WIDTH - 70,
					BookSetting.SCREEN_HEIGHT - 30, 0));
		} else {
			this.calcCornerXY(0, BookSetting.SCREEN_HEIGHT);
			this.doTouchEvent(MotionEvent.obtain(100, 100,
					MotionEvent.ACTION_DOWN, 0, BookSetting.SCREEN_HEIGHT, 0));
			this.doTouchEvent(MotionEvent.obtain(100, 100,
					MotionEvent.ACTION_MOVE, 0 + 30,
					BookSetting.SCREEN_HEIGHT - 10, 0));
			this.doTouchEvent(MotionEvent.obtain(100, 100,
					MotionEvent.ACTION_MOVE, 0 + 50,
					BookSetting.SCREEN_HEIGHT - 20, 0));
			this.doTouchEvent(MotionEvent.obtain(100, 100,
					MotionEvent.ACTION_UP, 0 + 70,
					BookSetting.SCREEN_HEIGHT - 30, 0));
		}
		...
		mc = new MyCount(HLSetting.FlipTime, 100);
		mc.start();
	}
...
//						BitmapManageUtil.currentscreen = null;
